...
function Page() {
    ...
    return (
        <main className="grid gap-4">
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 flex items-center gap-3">
                <SettingOutlined className="text-2xl! text-slate-400" />
                <div>
                    <h2 className="font-bold">การตั้งค่าระบบ</h2>
                    <p className="text-slate-500 text-sm">หน้าตั้งค่ายังเป็นแบบอ่านอย่างเดียว — แก้ไขได้ภายหลัง</p>
                </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {groups.map((g) => (
                    <div key={g.title} className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
