...
- **ห้ามเรียก `supabase.channel(topic)` + `supabase.removeChannel(ch)` ตรง ๆ ใน component สำหรับ topic ที่มีโอกาสมีผู้ subscribe หลายตัวพร้อมกัน** (เช่น `wallet-{userId}` ที่ทั้ง `AppHeader`+`/user/wallet`+`WalletListingBtn` ฟังพร้อมกัน หรือ `bid-{productId}` ที่การ์ด listing หลายใบของสินค้าเดียวกันฟังพร้อมกัน) — `@supabase/supabase-js` (`realtime-js`) **คืน channel instance เดิมถ้า topic ซ้ำ** ดังนั้นตัวที่ unmount ก่อนแล้ว `removeChannel()` เอง จะฆ่า channel ที่ component อื่นยังใช้อยู่ → broadcast ค้างจนต้อง reload
- ใช้ helper รวมศูนย์แทน: `subscribeWallet(userId, onUpdate)` ใน [wallet.service.js](../app/services/wallet.service.js) (channel เดียวต่อ user, module-level, ไม่ remove จนกว่า userId เปลี่ยน) และ `subscribeBidChannel(productId, onNewBid)` + `sendBidBroadcast(productId, payload)` ใน [bids.service.js](../app/services/bids.service.js) (channel ต่อ product, `Map` + ref-count + หน่วง 1 วิก่อน remove จริงกันเคส unsubscribe-แล้ว-subscribe-ซ้ำทันที)
- topic ใหม่ในอนาคตที่มีโอกาส subscribe จากหลาย component พร้อมกัน → ทำ pattern เดียวกัน (listeners `Set` + จัดการ lifecycle ของ channel ไว้ที่ helper ใน service file ไม่ใช่ใน component)
