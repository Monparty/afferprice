...
10. **RPC SECURITY DEFINER ใหม่ (โดยเฉพาะ RPC เงิน)** → ต้องปิดท้าย migration ด้วย `REVOKE ... FROM PUBLIC, anon, authenticated` + `GRANT ... TO service_role` เสมอ — Supabase **grant EXECUTE ให้ anon/authenticated อัตโนมัติ** (default privileges) ไม่ revoke = user login เรียกตรงจาก browser ได้ (บาง RPC รับ `p_user_id` เป็น argument → สวมรอยตัดเงินคนอื่นได้); ยกเว้น RPC ที่ตั้งใจให้ browser เรียก (เช่น `submit_kyc`) ซึ่ง**ต้องมี caller guard** `auth.uid()` เทียบกับ argument ภายในฟังก์ชัน. RPC เงินเก่าทั้งหมดถูก lock แล้วใน [20260704020000_lock_money_rpcs.sql](../supabase/migrations/20260704020000_lock_money_rpcs.sql)
