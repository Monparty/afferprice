...
## Add-product step 3 — บังคับจ่ายเงินค่าประกันการขายก่อนส่งตรวจสอบ + auto-save draft — 2026-07-17
- **Purpose:** เปลี่ยน flow step 3 ของ add-product — เดิม KYC approved แล้วกดปุ่ม "การบันทึกและส่งตรวจสอบสินค้า" ได้ทันที (จ่ายเงินค่าประกันการขายทีหลังตอนไหนก็ได้ก่อน admin approve) → เปลี่ยนเป็น **ต้องจ่ายเงินค่าประกันการขายก่อน ปุ่มส่งตรวจสอบถึงจะปลดล็อก**; เพื่อให้กล่องชำระเงิน (`ListingFeePayment`) มี `productId` ให้แสดงได้ทันทีที่เข้า step 3 (ไม่ต้องรอผู้ใช้กด "บันทึกเป็นฉบับร่าง" เอง) เพิ่ม auto-save draft เงียบ ๆ ตอนเข้า step 3
- **Location:**
  - `app/(authenticated)/user/add-product/components/CardAddProductPreview.jsx` — prop `isSubmitted` ใหม่ + คำนวณ `mainLabel`/`mainDisabled`/`handleMainClick` แทน nested ternary เดิมในปุ่มหลัก step 3
  - `app/(authenticated)/user/add-product/components/AddProductLayout.jsx` — `isSubmitted = watch("state") === "pending_review"`, `setValue("state", state)` หลัง upsert, `opts.silent` ใน `onSubmit`, `useEffect` + `autoDraftRef` auto-save draft
  - `app/(authenticated)/user/add-product/components/AddProductForm.jsx` — ข้อความ placeholder กล่องค่าธรรมเนียมตอน `!watchProductId`
- **Inputs/Outputs:**
  - flow ใหม่ step 3 (KYC `approved`): เข้า step 3 → auto-save draft เงียบ (`onSubmit("draft", {silent:true})`, ได้ `productId`) → กล่องชำระเงินค่าประกันการขายแสดงทันที → จ่ายเงิน (`isFeePaid`) → ปุ่มหลักปลดล็อกเป็น "การบันทึกและส่งตรวจสอบสินค้า" → กด → `onSubmit("pending_review")` → `state='pending_review'` (`isSubmitted=true`)
  - `CardAddProductPreview` ปุ่มหลัก step 3 ไล่ตามลำดับ: `isKyc==='pending'` → disabled "รอ admin ตรวจสอบ KYC"; `isKyc!=='approved'` (unknown/rejected) → เปิด `KycVerificationForm` modal; `isSubmitted` → disabled "รอ admin ตรวจสอบ"; `!isFeePaid` → disabled "ชำระเงินค่าประกันการขายก่อน"; มิฉะนั้น → enabled "การบันทึกและส่งตรวจสอบสินค้า" → `onSubmit("pending_review")`
  - ปุ่ม "บันทึกเป็นฉบับร่าง" ตอนนี้ `disabled={isFeePaid || isSubmitted}` (เดิม `disabled={isFeePaid}` อย่างเดียว)
  - แบนเนอร์เขียว (`isFeePaid`) แยกข้อความตาม `isSubmitted`: ยังไม่ส่ง → กดปุ่มส่งตรวจสอบต่อ; ส่งแล้ว → "รอ admin ตรวจสอบและอนุมัติ ไม่สามารถแก้ไข"
  - `AddProductLayout` `onSubmit(state, opts={})` เพิ่ม `opts.silent` — ถ้า `true` จะ `setValue("productId"/"state")` แล้ว `return` ก่อนถึง `notifySuccess`/ข้อความอื่น ๆ (ไม่มี toast ตอน auto-save); error ยัง `notifyError` ตามปกติ (ไม่ silence)
  - `useEffect` auto-draft (`autoDraftRef` กันยิงซ้ำ): เงื่อนไข `activeStep===3 && data?.is_kyc==='approved' && !effectiveProductId && !autoDraftRef.current` → ยิง `onSubmit("draft",{silent:true})` ครั้งเดียว
  - `AddProductForm.jsx` fee-box ตอน `!watchProductId` เปลี่ยนข้อความจาก "กรุณาบันทึกและส่งตรวจสอบสินค้าก่อน..." เป็น "กำลังเตรียมการชำระเงิน กรุณารอสักครู่..." (แสดงแค่ชั่วครู่ระหว่าง auto-save วิ่ง)
- **Gotchas:**
  - **gate นี้เป็น UI-only** — `upsertProduct` ใน `app/services/admin/products.service.js` (ทั้งฝั่ง server และ admin approve gate) **ยังไม่ได้บังคับว่าต้องจ่าย listing fee ก่อน transition เป็น `pending_review`** — เหมือน fee gate เดิมฝั่ง admin (ปุ่มอนุมัติ `disabled` ตาม `feePayment` แต่ไม่มี server-side check ใน `upsertProduct`) ยังเป็นช่องโหว่เดิมที่ไม่ได้แก้รอบนี้ — ถ้าต้องการกันแน่นต้องเพิ่ม check ใน `upsertProduct`/insert path ด้วย
  - **auto-save ล้มเหลว** (เช่น field จำเป็นไม่ครบตอนเข้า step 3 เร็วผิดปกติ) → `notifyError` ขึ้น error จริง (ไม่ silent) + fee-box ค้างข้อความ "กำลังเตรียมการชำระเงิน..." เพราะ `watchProductId` ยังว่าง; ปุ่ม "บันทึกเป็นฉบับร่าง" ยังกดเองได้เป็นทาง retry มือ
  - `isSubmitted` พึ่ง `watch("state")` ที่ถูก set ผ่าน `setValue("state", state)` เฉพาะตอน `onSubmit` สำเร็จ (ไม่ได้มาจาก DB fetch ตรง ๆ ตอน mount) — หน้า edit ได้ค่าเริ่มต้นถูกจาก `reset()` เพราะ `formatData` spread `...data` ที่มี `state` อยู่แล้ว (จาก `getProductById` เดิม)
  - `autoDraftRef` เป็น `useRef` module-level ต่อ mount ของ `AddProductLayout` เท่านั้น — สลับ step ไปมา (เช่น step 3 → step 2 → step 3 อีกรอบ) จะไม่ auto-save ซ้ำเพราะ `effectiveProductId` มีค่าแล้วตั้งแต่รอบแรก (guard `!effectiveProductId` เป็นตัวกันหลักจริง ๆ, ref กันแค่ race ภายใน effect เดียวกัน)
  - `npm run build` ผ่าน
  - **หมายเหตุ (ไม่เกี่ยวกับ feature นี้):** `git diff` รอบนี้มีการแก้ `app/components/utils/LandingPage2.jsx` ด้วย (ลบ section "ขายแล้วล่าสุด" ~31 บรรทัด) — ไม่ได้อยู่ใน scope ที่ผู้ใช้อธิบายตอนสั่ง "จด" รอบนี้ และดูเหมือนงานค้าง/ยังไม่เสร็จ (ไม่มี context ว่าทำไมถึงลบ) จึงไม่บันทึกเป็น entry แยก — ถ้าตั้งใจแก้จริงให้สั่ง "จด" อีกรอบพร้อมอธิบาย purpose
